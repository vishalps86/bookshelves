 ...
 * @url : https://github.com/verot/class.upload.php/blob/master/test/upload.php
 */
class upload {
     ...
     */
    function  __construct($file, $lang = 'en_GB',$updated_filename = '')  {
        $this->upload($file, $lang,$updated_filename);
    }
     ...
     */
    function upload($file, $lang = 'en_GB',$updated_filename) {
        ...
        if (!is_array($file)) {
                ...
                if (substr($file, 0, 4) == 'php:') {
                    ...
                    if ($this->uploaded) {
						if($updated_filename!='') {
								...
								$this->file_src_name       = basename($updated_filename);
								$this->log .= '- local file OK<br />';
								preg_match('/\.([^\.]*$)/', $this->file_src_name, $extension);
								if (is_array($extension) && sizeof($extension) > 0) {
									$this->file_src_name_ext      = strtolower($extension[1]);
            ...
            if ($this->uploaded) {
				if($updated_filename!='') {
				   ...
				   $this->file_src_name       = $updated_filename;
				}
				else {
				   $this->file_src_pathname   = $file['tmp_name'];
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    ...
                    while (@file_exists($this->file_dst_pathname)) {
                        ...
                        $this->file_dst_name = $this->file_dst_name_body . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
						$this->file_dst_name_time = time()."_".$this->file_dst_name_body . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
                        $cpt++;
                        $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
                    }
