...
class CI_Image_lib {
	...
	function image_process_gd($action = 'resize')
	{
		...
		if(
			...
			$exif = exif_read_data($this->full_src_path);
		...
		switch($this->orientation){
			...
			case 2:
					// horizontal flip
					$this->_image_mirror_gd($dst_img,'hor');
				break;
			case 3:
					// 180 rotate left
			...
			case 4:
					// vertical flip
					$this->_image_mirror_gd($dst_img,'vrt');
				break;
			case 5:
	...
	function image_process_imagemagick($action = 'resize')
	{
		...
		else  // Resize
		{
			...
			if(
				...
				$cmd .= " -resize ".$this->width."x \"$this->full_src_path\" \"$this->full_dst_path\" 2>&1";
			}else{
 ...
 * @return resource
 */
	private function _image_mirror_gd($src_img,$rotation_angle){
		$width  = $this->orig_width;
		$height = $this->orig_height;
		if ($rotation_angle == 'hor'){
			for ($i = 0; $i < $height; $i++){
				$left  = 0;
				$right = $width-1;
				while ($left < $right){
